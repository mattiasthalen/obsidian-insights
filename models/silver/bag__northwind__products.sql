...
SELECT
  CONCAT('northwind|product|', product_id::TEXT, '~epoch|valid_from|', _sqlmesh_valid_from::TEXT)::BLOB AS _hook__product__id__valid_from,
  ...
  CONCAT('northwind|category|', category_id) AS _hook__reference__id__category,
  product_id,
  supplier_id,
  category_id,
  product_name,

MODEL (
  name silver.pit__northwind__order_details,
  kind INCREMENTAL_BY_TIME_RANGE (
    time_column _sqlmesh_loaded_at
  ),
  grain (
    _hook__order_detail__id__valid_from, _hook__order__id__valid_from, _hook__product__id__valid_from
  )
);
...
SELECT
  ...
  bag__northwind__order_details._hook__order_detail__id,
  bag__northwind__order_details._hook__order__id,
  bag__northwind__order_details._hook__product__id,
  bag__northwind__order_details.order_id,
  bag__northwind__order_details.product_id,
  bag__northwind__order_details.unit_price,
  bag__northwind__order_details.quantity,
  bag__northwind__order_details.discount,
  bag__northwind__order_details.discount__v_double,
  bag__northwind__order_details._dlt_load_id,
  bag__northwind__order_details._dlt_id,
  bag__northwind__order_details._dlt_extracted_at,
  bag__northwind__order_details._sqlmesh_hash_diff,
  ...
  --ROW_NUMBER() OVER (PARTITION BY bag__northwind__order_details._hook__order_detail__id ORDER BY _sqlmesh_valid_from) AS _sqlmesh_version,
  --ROW_NUMBER() OVER (PARTITION BY bag__northwind__order_details._hook__order_detail__id ORDER BY _sqlmesh_valid_from DESC) = 1 AS _sqlmesh_is_current_record,

  bag__northwind__order_details._sqlmesh_source_system,
  bag__northwind__order_details._sqlmesh_source_table
...
WHERE
  bag__northwind__order_details._sqlmesh_loaded_at BETWEEN @start_ts AND @end_ts